...
如果低版本升级高版本出现异常，建议先使用 **6.高级/危险操作 -> 1.卸载脚本** 卸载后重新安装，再确认是否仍可复现。
...
例如：`padm -> 1.安装/重装节点 -> 2.自定义安装 -> 7.VLESS Reality Vision`，按提示填写 entry、Reality target 和 SNI，安装后进入 `2.查看/管理订阅` 查看账号。
...
```text
例如：1.完整安装 / 2.自定义安装 / 3.无域名 Reality 安装
```
