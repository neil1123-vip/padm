...
normalizeSubscriptionGroupsState() {
    ...
    jq --argjson schemaVersion "${schemaVersion}" '
      ...
      def normalize_source:
        ...
        .scheme = (($source.scheme // "https") | tostring) |
...
restoreSubscriptionGroupsBackup() {
    ...
    local tmpFile
    stateFile=$(subscriptionGroupsFile)
    tmpFile="${stateFile}.restore.tmp"
    [[ -f "${backupFile}" ]] || return 1
    jq empty "${backupFile}" >/dev/null 2>&1 || return 1
    mkdir -p "$(dirname "${stateFile}")"
    cp "${backupFile}" "${tmpFile}" && mv "${tmpFile}" "${stateFile}"
    migrateSubscriptionGroupsState
}
...
addSubscriptionSourceState() {
    ...
    local groupId
    groupId=$(activeSubscriptionGroupId)
    subscriptionGroupsStateWrite --arg groupId "${groupId}" --arg id "${id}" --arg name "${name}" --arg scheme "${scheme}" --arg host "${host}" --argjson port "${port}" '
      .groups |= map(if .id == $groupId then
        if any(.sources[]?; .id == $id) then . else
          .sources += [{"id": $id, "name": $name, "role": "secondary", "scheme": $scheme, "host": $host, "port": $port, "enabled": true, "sync_status": "pending"}]
        end
      else . end)'
}
