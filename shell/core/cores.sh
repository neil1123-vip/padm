...
singBoxCompatibilityAuditScanJsonFile() {
    ...
    if ! findings=$(jq -r --argjson defaultHttpClientConfigured "${defaultHttpClientConfigured}" '
        ...
        [
            ...
            if any(dns_configs; uses_legacy_dns_strategy) then "dns-rule-strategy" else empty end,
            if any(dns_configs; uses_legacy_dns_rule_set_ip_cidr_accept_empty) then "dns-rule-set-ip-cidr-empty" else empty end,
            if any(dns_configs; uses_legacy_dns_address_filter) then "dns-address-filter" else empty end,
    ...
    while IFS= read -r finding; do
        ...
        tls-acme) coreCompatibilityAuditWarn "${warnFile}" "${logFile}" "检测到已弃用的内嵌 tls.acme，请迁移到 certificate_provider（1.16 将移除）：${file}" || return 1 ;;
        store-rdrc) coreCompatibilityAuditWarn "${warnFile}" "${logFile}" "检测到已弃用的 store_rdrc，请改用 store_dns（1.16 将移除）：${file}" || return 1 ;;
...
appendSingBoxCompatibilityHints() {
    local logFile=$1
    [[ -f "${logFile}" ]] || return 0
    if grep -Eqi 'legacy DNS servers|legacy special outbounds|legacy domain strategy|domain_resolver|default_domain_resolver|wireguard.*outbound|domain_strategy|independent_cache|download_detour|default_http_client|隐式默认 HTTP client|默认 HTTP client|tls\.acme|store_rdrc|certificate_provider|store_dns|rule_set_ip_cidr_accept_empty|match_response|DNS 规则 strategy' "${logFile}"; then
        {
            printf '\n[padm 兼容性提示]\n'
            ...
            printf -- '- sing-box 1.14 已弃用内嵌 tls.acme 与 store_rdrc；请迁移到 certificate_provider 与 store_dns。\n'
            printf -- '- sing-box 1.14 已弃用 DNS 规则 strategy、rule_set_ip_cidr_accept_empty 及未带 match_response 的 ip_cidr/ip_is_private；请迁移到 domain_resolver 或 evaluate + match_response。\n'
        } >>"${logFile}"
    fi
}
