...
订阅系统按服务器角色组织，主控和被控首页直接提供常用动作，低频维护函数保留为兼容入口。

| 状态 | 菜单形态 | 适合做什么 |
| --- | --- | --- |
| 🟡 未初始化 | `本机单独使用` / `这台作为主控` / `这台作为被控` | 单机可直接管理本机订阅；需要多服务器时再选择主控或被控。 |
| 🟢 主控 | 主控首页直接提供订阅管理、同步、协同和控制面动作 | 统一查看本机自用与分享订阅，创建分享订阅、添加被控服务器、执行同步和限额治理。 |
| 🔵 被控 | 被控首页直接提供接入、状态、凭据和控制面动作 | 粘贴主控邀请、提供本机节点给主控、查看 WireGuard 与同步状态。 |

主控和本机模式首页都直接提供统一的 `订阅同步` 菜单：立即完整同步、开启/关闭自动同步、设置间隔、状态与排障、用量与限额。自动同步开关同时控制配置变更后的即时同步和 cron；关闭后仍可随时手动完整同步。

首页的 `订阅管理` 统一处理链接刷新、分享订阅维护和用量入口。管理员自用订阅仍由本机协议配置自动维护，不写入 `user_groups`、不参与额度治理，也不会通过主控同步下发；统一的是管理入口和发布流程。
...
1. 在本机或主控首页进入 `安装/更新订阅服务`。
2. 在同一首页进入 `新建并发布订阅`，使用英文、数字或短横线 ID，例如 `team-a`。
3. 按向导选择可用服务器源和流量上限。
...
1. 主控服务器从主控首页进入 `主控建链向导` 完成初始化，再进入 `添加/移除被控服务器` -> `创建被控邀请`，输入一次被控别名。主控会自动预留 WireGuard 地址。
2. 被控服务器进入 `接入主控`，粘贴邀请即可完成初始化和主控 Peer 导入；无需填写地址。成功后复制一次接入回执。
3. 回到主控服务器，从主控首页进入 `添加/移除被控服务器` -> `完成被控接入`，粘贴回执即可按预留别名完成 Peer、服务器源和控制 Token 写入，并自动检查本次来源的健康状态。
