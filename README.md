...
6. **安装完成**：回到主菜单选择 `订阅与用户` -> `快速开始`；自己用选 `我自己用`，给别人开链接选 `给别人开订阅`。
...
| 订阅与用户 | 快速开始、我的订阅、给别人开订阅、用量限额、同步备份、多服务器主控/被控和高级诊断。 |
...
1. **快速开始**：按“我自己用 / 给别人开订阅 / 治理用量 / 接入多服务器”串起常见闭环。
2. **我的订阅**：安装/更新订阅发布服务，查看/刷新自用订阅链接、可用服务器和自用流量。
3. **给别人开订阅**：一键创建用户订阅并同步，生成托管账号 `sub_<ID>`，再查看用户订阅链接。
4. **用量与限额**：刷新并显示总览，查看用户/服务器用量，预览和执行超限处理。
5. **同步与备份**：立即同步、查看本机/远程计划、配置自动同步、创建/恢复备份，必要时显式重建 `groups.json`。
6. **多服务器：主控**：本机作为主控，初始化控制面、添加被控、健康检查、查看同步结果。
7. **多服务器：被控**：本机作为被控，导入主控凭据并输出被控接入凭据。
8. **高级诊断**：集中查看订阅服务、WireGuard 控制面、远端健康、同步错误、cron 和兼容控制面入口。

给用户创建订阅的推荐流程：

1. `快速开始` -> `给别人开订阅`，或直接进入 `给别人开订阅` -> `一键创建并生成链接`；ID 使用英文、数字或短横线，例如 `team-a`。
...
1. 在主控服务器进入 `多服务器：主控` -> `主控建链向导`，初始化本机为主控，并复制“本机主控接入凭据”。
2. 在被控服务器进入 `多服务器：被控` -> `被控加入向导`，初始化本机为被控，导入主控接入凭据，再复制“本机被控接入凭据”。
3. 回到主控服务器进入 `多服务器：主控` -> `添加/移除被控服务器`，粘贴被控接入凭据并设置本地别名。
