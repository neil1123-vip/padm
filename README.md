...
6. **安装完成**：回到主菜单选择 `订阅与用户` -> `快速开始`；自己用选 `我自己用`，给别人开链接选 `给别人用`。
...
| 订阅与用户 | 主控/未初始化时显示完整入口；初始化为被控后收敛为快速开始、多服务器协同和高级诊断。 |
...
1. **快速开始**：用于按“我自己用 / 给别人用 / 查看用量与限额 / 接入多服务器”串起常见闭环。
2. **我自己用**：用于安装/更新订阅发布服务，并刷新查看自用订阅链接、可用服务器和自用流量。
3. **给别人用**：用于新建分享订阅，或处理已有订阅的链接、节点范围、额度和启停。
4. **多服务器协同**：用于主控/被控接入、添加被控、更新凭据和查看协同状态；低频维护收在细项里。
5. **运行与维护**：用于刷新运行总览、立即同步和查看运行状态；低频设置、备份恢复与细项治理收在细项里。
6. **高级诊断**：用于集中排查故障，查看诊断状态、清除同步错误和进入兼容控制面入口。

当前机器初始化为被控后，`订阅与用户` 顶层会自动收敛为 `快速开始 / 多服务器协同 / 高级诊断`，隐藏 `我自己用 / 给别人用 / 运行与维护` 等主控侧入口。

给用户创建订阅的推荐流程：

1. `快速开始` -> `给别人用`，或直接进入 `给别人用` -> `新建并发布订阅`；ID 使用英文、数字或短横线，例如 `team-a`。
2. 按向导设置授权服务器（默认 `main`）和流量上限。
3. 确认摘要后立即执行同步，托管账号 `sub_<ID>` 会写入核心配置。
4. 同步完成后可直接拿到用户订阅链接；如果当时跳过了订阅服务安装，后续可在 `给别人用` -> `查看并处理已有订阅` 中补生成。

多服务器同步推荐流程：

1. 在主控服务器进入 `多服务器协同` -> `主控建链向导`，初始化本机为主控，并复制“本机主控接入凭据”。
2. 在被控服务器进入 `多服务器协同` -> `被控加入向导`，初始化本机为被控，导入主控接入凭据，再复制“本机被控接入凭据”。
3. 回到主控服务器进入 `多服务器协同` -> `添加/移除被控服务器`，粘贴被控接入凭据并设置本地别名。
4. 进入 `多服务器协同` -> `查看协同状态`，连续确认接入凭据、服务器源、健康检查和最近同步结果；确认无误后进入 `运行与维护` -> `查看运行状态` 或直接 `立即执行同步`。
