...
1. 主控服务器进入 `多服务器协同` -> `主控建链向导`，初始化主控并复制主控接入凭据。
2. 被控服务器进入 `接入主控`，导入主控凭据，再复制本机被控接入凭据。
3. 回到主控服务器，进入 `多服务器协同` -> `添加/移除被控服务器`，粘贴被控凭据并设置别名。
4. 进入 `多服务器协同` -> `查看协同状态`，确认凭据、服务器源、健康检查和最近同步结果。
5. 客户端订阅继续走公网 HTTPS；服务器间控制接口只走 WireGuard 内网。
