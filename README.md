...
6. **安装完成**：回到主菜单选择 `订阅与用户`。未初始化先选 `这台作为主控` 或 `这台作为被控`；主控用 `发布订阅` 和 `多服务器协同`，被控用 `接入主控` 和 `查看本机状态`。
...
| 订阅与用户 | 按角色切成三态：未初始化只显示 `这台作为主控 / 这台作为被控`；主控显示 `发布订阅 / 多服务器协同 / 主控维护与排障`；被控显示 `接入主控 / 查看本机状态 / 被控维护与排障`。 |
...
主菜单 `订阅与用户` 现在按服务器角色组织，而不是按旧任务分组平铺：

1. **未初始化**：只显示 `这台作为主控 / 这台作为被控 / 返回主菜单`，先完成角色初始化。
2. **主控首页**：只显示 `发布订阅 / 多服务器协同 / 主控维护与排障 / 返回主菜单`。
3. **被控首页**：只显示 `接入主控 / 查看本机状态 / 被控维护与排障 / 返回主菜单`。

主控侧结构：

1. `发布订阅`：用于安装/更新订阅发布服务、刷新自用订阅、创建分享订阅、维护已有订阅、查看可用服务器和自用流量。
2. `多服务器协同`：用于主控建链、添加/移除被控服务器、更新被控服务器凭据和查看协同状态。
3. `主控维护与排障`：用于刷新运行总览、立即执行同步、查看运行状态、处理用量与限额、自动同步、状态备份恢复、控制面细节和清除同步错误。

被控侧结构：

1. `接入主控`：直接执行被控加入向导，初始化被控、导入主控凭据并输出本机被控接入凭据。
2. `查看本机状态`：连续展示角色摘要、本机被控接入凭据、WireGuard 状态和最近同步结果。
3. `被控维护与排障`：只保留导入/更新主控接入凭据、查看控制面与 Peer 细节、重写配置并重启被控控制面、关闭被控控制面。

给用户创建订阅的推荐流程：

1. 在主控服务器进入 `发布订阅` -> `新建并发布订阅`；ID 使用英文、数字或短横线，例如 `team-a`。
2. 按向导设置授权服务器（默认 `main`）和流量上限。
3. 确认摘要后立即执行同步，托管账号 `sub_<ID>` 会写入核心配置。
4. 同步完成后可直接拿到用户订阅链接；如果当时跳过了订阅服务安装，后续可在 `发布订阅` -> `查看并处理已有订阅` 中补生成。

多服务器同步推荐流程：

1. 在主控服务器进入 `多服务器协同` -> `主控建链向导`，初始化本机为主控，并复制“本机主控接入凭据”。
2. 在被控服务器进入 `接入主控`，初始化本机为被控，导入主控接入凭据，再复制“本机被控接入凭据”。
3. 回到主控服务器进入 `多服务器协同` -> `添加/移除被控服务器`，粘贴被控接入凭据并设置本地别名。
4. 进入 `多服务器协同` -> `查看协同状态`，连续确认接入凭据、服务器源、健康检查和最近同步结果；确认无误后进入 `主控维护与排障` -> `查看运行状态` 或直接 `立即执行同步`。
...
状态备份与恢复只作用于 `/etc/padm/subscribe_groups/groups.json`。恢复备份或重建状态前脚本会先自动创建当前状态备份，并要求输入 `yes`；升级或普通打开菜单不会自动重置订阅状态。
