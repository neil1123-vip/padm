...
状态：本地实施与自动回归验收已完成；真实双被控、混合版本和 systemd/WireGuard 现场验收尚未执行。

基线：`main` / `68d6bec`（`v2.3.5`）。

实施分支：`codex/subscription-sync-menu-simplify`；本次实施作为独立 `feat(subscription)` 提交，尚未合并、推送或发布。
...
执行这些项目会写入并重启真实 VPS，需另行取得生产操作授权。当前已完成本地实现、自动验收和任务分支独立提交；合并、推送、发布与生产部署均未执行。
