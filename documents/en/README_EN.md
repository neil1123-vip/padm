...
Automatic releases run only when installation entries, runtime scripts, Docker configuration, image inputs, or the version lock change on `main`. Documentation, tests, and CI files alone do not increment the version. Release runs process the latest `main` serially and publish only after all three assets have been uploaded and their digests checked. Run the `Release` workflow to retry or publish explicitly, or `Docker CI` for validation.
